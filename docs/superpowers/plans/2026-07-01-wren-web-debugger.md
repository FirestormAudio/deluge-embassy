...
- [ ] **Step 2b (handle-type fix — REQUIRED for the ungated build):** the generic `*_impl` bodies (which stay ungated) currently convert stored handles to `*mut wren_sys::WrenHandle`. With `wren-sys` gated off, `WrenHandle` won't resolve. So change the stored callback-handle FIELDS (`Metro.cb`, `MidiState.*`, `UiState.*`, `CALL_HANDLE`, in `bindings.rs`) from `*mut WrenHandle` to the dwc `Handle` newtype, and drop the `Handle`↔`*mut WrenHandle` conversions from the generic bodies (store/read `Handle` directly; the `SlotApi` handle methods already speak `Handle`). The generic bodies and state structs must have ZERO `wren_sys` references after this. Only the gated `extern "C"` wrappers/`Vm` impl may name `wren_sys` types. (`engine.rs` has no handle code — unaffected.)

- [ ] **Step 3: Implement the gating** — in `Cargo.toml`: `wren-sys = { path = "../../wren-sys", optional = true }` + the `[features]` block above. Gate the module-level `use wren_sys::{...}` in `bindings.rs` too (it names `Vm`/`WrenVM`/`ClassEntry`/`MethodEntry`/`WrenHandle`, all backend-only after Step 2b). Add `#[cfg(feature = "wren-sys-backend")]` to: the `mod slotapi_wrensys;` line and the module; every `extern "C"` wrapper + `METHODS`/`CLASSES`/`boot`/`prelude_ptr` AND the public entry points `tick`/`midi_rx`/`enc_turn`/`input_dispatch` in `bindings.rs` (plus their `pub use` re-exports in `lib.rs`); and the `test_support` module. `engine.rs` (pure DSP), the generic `*_impl` bodies, `SlotApi`, and the shared types stay ungated.
