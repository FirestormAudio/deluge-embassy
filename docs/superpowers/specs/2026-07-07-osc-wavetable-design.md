...
- **First real Pool→node integration.** The dynamic path allocates a Pool region,
  ...
  `Node.free()`.
- **`no_std`-pure player, opt-in builder.** The kernel does mip-select +
  interpolation only (no FFT, no alloc). The FFT-based mip **builder** (`mipgen`)
  is a separate std/`alloc` crate.
...
- **Feedback FM on wavetables** (phase-feedback on a table; niche — revisit with
  morph).
- Runtime table building on a `no_std`-*without-alloc* target (such a target uses
  named static tables only; documented, not a defect).
- SIMD; bumping `MAX_INPUTS`.
...
### 3.1 `mipgen` — a separate std/`alloc` builder crate

Reuses the QA harness's real FFT: base single-cycle → FFT → for each octave mip,
zero every harmonic above that octave's safe count → IFFT → one mip level. It is
**opt-in** and lives outside the `no_std` kernel:

- The **player** (kernel `WtOsc`) is pure `no_std`.
- The **builder** needs alloc + FFT. A `no_std`-*without-alloc* project uses named
  static tables only; it just can't build user tables at runtime. Documented.
...
**Builder (`mipgen` crate, std side):**

- **Band-limit correctness.** FFT each generated mip; assert energy above its
  target harmonic cutoff ≈ 0 (the builder actually band-limits).
...
- `mipgen` crate placement (standalone vs alongside `deluge-dsp-test`) and whether
  named tables are generated by a committed `gen-tables` binary or a `build.rs`.
